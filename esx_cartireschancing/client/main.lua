...
AddEventHandler('gln_oponazapasowa:uzyj', function()
	ESX.ShowHelpNotification('~r~接近損壞的車輪!')
	animacja()
end)
		...
		if vehicle ~= 0 then
			...
			if closestTire ~= nil then
				if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
					...
					Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, "~r~該輪胎需要修理!")
				end
			end
		end
	end
		...
		if DoesEntityExist(vehicle) then
			...
			TriggerServerEvent('gln_oponazapasowa:usunOpone')
				...
				Citizen.CreateThread(function()
					...
					TriggerEvent("pNotify:SendNotification", {text = "您正在更換輪胎!", type = "info", timeout = 5000, layout = "centerLeft"})
					...
					if CurrentAction ~= nil then
						...
						if closestTire ~= nil then
							...
							TriggerEvent("pNotify:SendNotification", {text = "您已完成輪胎更換!", type = "success", timeout = 2000, layout = "centerLeft"})
						end
					end
